...
const DashboardPreview = () => (
  <div className="relative w-full max-w-4xl mx-auto mt-16 aspect-[16/10] bg-[#1a1033] border border-white/10 rounded-xl shadow-2xl shadow-purple-900/50 overflow-hidden flex flex-col animate-float">
    {/* Header Fake */}
    ...
    <div className="flex-1 flex">
      ...
      <div className="w-48 border-r border-white/5 bg-white/[0.02] p-4 hidden md:flex flex-col gap-3">
        <div className="h-8 w-full bg-purple-500/20 rounded-lg mb-4"></div>
      ...
      <div className="flex-1 p-6 relative overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="h-32 rounded-xl bg-gradient-to-br from-purple-500/10 to-transparent border border-white/5 p-4">
            <div className="w-8 h-8 rounded bg-purple-500/20 mb-2"></div>
            <div className="h-4 w-1/2 bg-white/10 rounded mb-2"></div>
            <div className="h-2 w-3/4 bg-white/5 rounded"></div>
          </div>
          <div className="h-32 rounded-xl bg-gradient-to-br from-pink-500/10 to-transparent border border-white/5 p-4">
        ...
        {/* Bottom Section */}
        <div className="h-40 rounded-xl bg-white/5 border border-white/5 p-4 flex gap-4 items-center">
             <div className="flex-1 space-y-2">
                 <div className="h-4 w-full bg-white/10 rounded"></div>
                 <div className="h-4 w-2/3 bg-white/10 rounded"></div>
             </div>
             <div className="w-32 h-32 rounded-lg bg-gradient-to-tr from-purple-600 to-indigo-600 opacity-50 blur-xl"></div>
        </div>
...
const Node = ({ icon: Icon, position, delay, color }: { icon: any, position: string, delay: string, color: string }) => (
  <div className={`absolute ${position} hidden lg:flex flex-col items-center justify-center animate-pulse-slow ${delay} z-20`}>
    <div className={`w-12 h-12 rounded-xl bg-[#0B0518] border border-white/10 shadow-[0_0_15px_rgba(168,85,247,0.3)] flex items-center justify-center relative group`}>
  ...
  return (
    <section className="relative pt-32 pb-20 md:pt-48 md:pb-32 overflow-hidden bg-hero-glow">
      ...
      <div className="max-w-7xl mx-auto px-6 text-center relative z-10">
        ...
        {/* Badge Superior */}
        <div className="reveal inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-purple-200 text-xs font-medium mb-8 hover:bg-white/10 transition-colors cursor-default backdrop-blur-md">
        ...
        <h1 className="reveal delay-100 relative text-4xl md:text-6xl lg:text-7xl font-extrabold tracking-tight leading-[1.1] mb-6 text-white drop-shadow-2xl">
          Transforme Produtos<br/> em <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-purple-100 to-purple-300">Dinheiro.</span> <br/>
        ...
        <div className="reveal delay-300 flex flex-col sm:flex-row items-center justify-center gap-4 mb-16 relative z-20">
          <button 
            onClick={onLogin}
            className="px-8 py-4 rounded-full bg-purple-600 hover:bg-purple-500 text-white font-bold text-lg transition-all shadow-[0_0_20px_rgba(147,51,234,0.3)] hover:shadow-[0_0_40px_rgba(147,51,234,0.6)] transform hover:-translate-y-1 w-full sm:w-auto"
          >
            Quero Vender Mais
          </button>
          <button 
            onClick={onLogin}
            className="px-8 py-4 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold backdrop-blur-sm transition-all w-full sm:w-auto flex items-center justify-center"
          >
            Ver Demonstração
