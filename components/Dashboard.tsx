      ...
      <div className="bg-[#1a1025] border border-white/10 p-6 rounded-2xl max-w-sm w-full shadow-2xl shadow-purple-900/20 transform scale-100 animate-in zoom-in-95 duration-300">
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <div className="flex gap-3 w-full mt-2">
            <button onClick={onCancel} className="flex-1 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-bold text-sm transition-colors">
              Cancelar
            </button>
            <button onClick={onConfirm} className="flex-1 py-3 rounded-xl bg-red-600 hover:bg-red-500 text-white font-bold text-sm transition-colors shadow-lg shadow-red-900/20">
              Sim, Excluir
            </button>
          </div>
        </div>
...
export const Dashboard: React.FC<DashboardProps> = ({ onLogout, userEmail }) => {
  ...
  // --- SEPARAÇÃO DE ESTADO POR MÓDULO ---
  ...
  const handleThemeChange = (newTheme: typeof BACKGROUNDS[0]) => {
     ...
     const newPrefs = { ...themePrefs, [activeModule]: newTheme.id, 'global': newTheme.id };
  ...
  // --- HANDLER DE INPUT SCOPED ---
  const handleInputChange = (field: string, value: string) => {
      setFormDataByModule(prev => ({
          ...prev,
          [activeModule]: {
              ...prev[activeModule],
              [field]: value
  ...
  // --- FUNÇÃO PARA DELETAR ITEM COM MODAL ---
  ...
  const handleGenerate = async () => {
    ...
    if (activeModule !== 'studio' && activeModule !== 'settings' && activeModule !== 'history') {
         if (fields.length === 0 || fields.every(f => !String(f).trim())) {
             setError("Por favor, preencha pelo menos um campo de texto.");
             return;
         }
    }
    ...
    try {
        ...
        // INSTRUÇÃO GLOBAL ANTI-MARKDOWN
        const antiMarkdown = "IMPORTANTE: Retorne APENAS texto puro dentro dos valores JSON. NÃO use formatação markdown como **negrito**, ## títulos, ou *itálico*. O texto deve ser limpo e pronto para uso.";
        ...
        switch (activeModule) {
            ...
            case 'video_script':
                prompt = `Atue como um Roteirista Viral do TikTok. ${antiMarkdown}
                Produto: "${data.productName}".
            ...
            case 'email_marketing':
                prompt = `Atue como Especialista em E-mail Marketing Direto. ${antiMarkdown}
                Produto: "${data.productName}".
            ...
            case 'influencer_dm':
                prompt = `Atue como Head de Parcerias. ${antiMarkdown}
                Produto: "${data.productName}".
                Nicho: ${data.niche || 'Beleza'}.
                Meu Nome: ${data.userName || 'Representante'}.
                Nome da Minha Empresa: ${data.companyName || 'Sua Loja'}.
                Proposta: ${data.proposal || 'Permuta'}.
                Retorne JSON: { "script_formal": "...", "script_casual": "...", "dicas_negociacao": "..." }`;
                break;

            case 'blog_post':
                prompt = `Atue como Especialista SEO Sênior. ${antiMarkdown}
                Assunto: "${data.topic}".
            ...
            case 'product_desc':
                prompt = `Atue como Especialista em Conversão de E-commerce. ${antiMarkdown}
                Produto: "${data.productName}".
            ...
            case 'persona':
                prompt = `Atue como Cientista de Dados de Marketing. ${antiMarkdown}
                Produto: "${data.productName}".
            ...
            case 'roas_analyzer':
                prompt = `Atue como CFO de E-commerce. ${antiMarkdown}
                Custo: ${data.cost}. Venda: ${data.salePrice}. CPA: ${data.cpa || '25% do preço'}.
                Retorne JSON: { "lucro_bruto": "...", "margem_porcentagem": "...", "analise_viabilidade": "...", "sugestao_preco": "..." }`;
                break;

             case 'policy_gen':
                prompt = `Advogado Especialista em E-commerce. ${antiMarkdown}
                Loja: "${data.storeName}". Email: ${data.supportEmail}. Prazo: ${data.deliveryTime}.
                Retorne JSON: { "termos_servico_resumo": "...", "politica_privacidade_resumo": "...", "politica_reembolso_resumo": "..." }`;
                break;

            case 'headline_optimizer':
                 prompt = `Copywriter Lendário. ${antiMarkdown}
                 Produto: "${data.productName}". Promessa: ${data.promise}.
        ...
        if (activeModule !== 'studio') {
            ...
            try {
                // Limpeza Extra de Markdown caso a IA desobedeça
                const cleanText = textStr.replace(/```json/g, '').replace(/```/g, '').trim();
                const parsed = JSON.parse(cleanText);
                
                // Função recursiva para limpar strings dentro do objeto
                const cleanObjectStrings = (obj: any): any => {
                    if (typeof obj === 'string') {
                        return obj.replace(/\*\*/g, '').replace(/##/g, '').replace(/__/g, '');
                    } else if (Array.isArray(obj)) {
                        return obj.map(cleanObjectStrings);
                    } else if (typeof obj === 'object' && obj !== null) {
                        const newObj: any = {};
                        for (const key in obj) {
                            newObj[key] = cleanObjectStrings(obj[key]);
                        }
                        return newObj;
                    }
                    return obj;
                ...
                setResult(cleanObjectStrings(parsed));
            } catch (e) {
                setResult({ "Resposta": textStr.replace(/\*\*/g, '') }); 
            }
        }
                ...
                <>
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase flex justify-between">
                            <span>Foto (Opcional - Apenas Simulador)</span> {adImage && <span className="text-green-400 text-[9px] font-bold bg-green-900/30 px-2 rounded">OK</span>}
                        </label>
                        <div onClick={() => fileInputRef.current?.click()} className="w-full h-16 border-2 border-dashed border-slate-700 hover:border-purple-500 hover:bg-slate-800/50 rounded-xl flex items-center justify-center cursor-pointer gap-2 transition-all group">
                             <Upload className="w-5 h-5 text-slate-500 group-hover:text-purple-400 transition-colors"/> 
                             <span className="text-xs font-bold text-slate-500 group-hover:text-white transition-colors">Carregar Imagem</span>
                        </div>
            ...
            return (
                <>
                    <div className="space-y-2">
                        ...
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white focus:border-purple-500 outline-none" placeholder="Ex: Escova Alisadora" value={dynamicFormData.productName || ''} onChange={e => handleInputChange('productName', e.target.value)} />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                         <div className="space-y-2">
                             <label className="text-[10px] font-bold text-slate-500 uppercase">Duração</label>
                             <select className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white focus:border-purple-500 outline-none" value={dynamicFormData.duration || '30s'} onChange={e => handleInputChange('duration', e.target.value)}>
                                 <option value="15s">15 Segundos (Stories)</option>
                                 <option value="30s">30 Segundos (Reels/TikTok)</option>
                                 <option value="60s">1 Minuto (Shorts)</option>
                             </select>
                        </div>
                        <div className="space-y-2">
                             <label className="text-[10px] font-bold text-slate-500 uppercase">Estilo</label>
                             <select className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white focus:border-purple-500 outline-none" value={dynamicFormData.style || 'Engraçado'} onChange={e => handleInputChange('style', e.target.value)}>
                                 <option value="Engraçado">Divertido / Viral</option>
                                 <option value="UGC">Depoimento Real (UGC)</option>
                                 <option value="Problema-Solução">Problema & Solução</option>
                                 <option value="ASMR">Satisfatório / ASMR</option>
                             </select>
                        </div>
                    </div>
                </>
             ...
             return (
                <>
                    <div className="space-y-2">
                        ...
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Smartwatch X8" value={dynamicFormData.productName || ''} onChange={e => handleInputChange('productName', e.target.value)} />
                    </div>
                    <div className="space-y-2">
                         <label className="text-[10px] font-bold text-slate-500 uppercase">Objetivo do E-mail</label>
                         <select className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" value={dynamicFormData.goal || 'Recuperar Carrinho'} onChange={e => handleInputChange('goal', e.target.value)}>
                     ...
                     <div className="space-y-2">
                        ...
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: 10% OFF ou Frete Grátis" value={dynamicFormData.discount || ''} onChange={e => handleInputChange('discount', e.target.value)} />
                    </div>
                </>
             );
        case 'influencer_dm':
             return (
                <>
                    <div className="space-y-2">
                        ...
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Kit de Maquiagem" value={dynamicFormData.productName || ''} onChange={e => handleInputChange('productName', e.target.value)} />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-500 uppercase">Seu Nome</label>
                            <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: João Silva" value={dynamicFormData.userName || ''} onChange={e => handleInputChange('userName', e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-slate-500 uppercase">Sua Empresa</label>
                            <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Beauty Store" value={dynamicFormData.companyName || ''} onChange={e => handleInputChange('companyName', e.target.value)} />
                        </div>
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Nicho do Influencer</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Moda, Gamer, Fitness..." value={dynamicFormData.niche || ''} onChange={e => handleInputChange('niche', e.target.value)} />
                    </div>
                     <div className="space-y-2">
                         <label className="text-[10px] font-bold text-slate-500 uppercase">Tipo de Proposta</label>
                         <select className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" value={dynamicFormData.proposal || 'Permuta'} onChange={e => handleInputChange('proposal', e.target.value)}>
             ...
             return (
                <>
                    <div className="space-y-2">
                        ...
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Como escolher o melhor tênis" value={dynamicFormData.topic || ''} onChange={e => handleInputChange('topic', e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Palavra-chave (SEO)</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: tênis de corrida barato" value={dynamicFormData.keyword || ''} onChange={e => handleInputChange('keyword', e.target.value)} />
                    </div>
                </>
             );
        case 'product_desc':
             return (
                <>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Nome do Produto</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Fone Bluetooth Pro" value={dynamicFormData.productName || ''} onChange={e => handleInputChange('productName', e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Características Principais</label>
                        <textarea className="w-full h-24 bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none resize-none" placeholder="Ex: À prova d'água, bateria de 24h, cancelamento de ruído..." value={dynamicFormData.features || ''} onChange={e => handleInputChange('features', e.target.value)} />
                    </div>
                     <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Benefício Principal (A "promessa")</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Ouvir música sem interrupções" value={dynamicFormData.benefit || ''} onChange={e => handleInputChange('benefit', e.target.value)} />
                    </div>
                </>
             );
        case 'persona':
             return (
                <>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Produto</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Cinta Modeladora" value={dynamicFormData.productName || ''} onChange={e => handleInputChange('productName', e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Categoria</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Beleza e Estética" value={dynamicFormData.category || ''} onChange={e => handleInputChange('category', e.target.value)} />
                    </div>
                     <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Preço do Produto</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: R$ 129,90" value={dynamicFormData.price || ''} onChange={e => handleInputChange('price', e.target.value)} />
                    </div>
                </>
             );
        case 'roas_analyzer':
             return (
                <>
                    <div className="space-y-2">
                        ...
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: R$ 30,00" value={dynamicFormData.cost || ''} onChange={e => handleInputChange('cost', e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Preço de Venda (Site)</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: R$ 99,00" value={dynamicFormData.salePrice || ''} onChange={e => handleInputChange('salePrice', e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Custo Estimado Marketing</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: R$ 25,00" value={dynamicFormData.cpa || ''} onChange={e => handleInputChange('cpa', e.target.value)} />
                    </div>
                </>
             );
        case 'policy_gen':
             return (
                <>
                    <div className="space-y-2">
                        ...
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Minha Loja Inc" value={dynamicFormData.storeName || ''} onChange={e => handleInputChange('storeName', e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">E-mail de Suporte</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: contato@minhaloja.com" value={dynamicFormData.supportEmail || ''} onChange={e => handleInputChange('supportEmail', e.target.value)} />
                    </div>
                     <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Prazo de Entrega Médio</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: 10 a 20 dias úteis" value={dynamicFormData.deliveryTime || ''} onChange={e => handleInputChange('deliveryTime', e.target.value)} />
                    </div>
                </>
             );
        case 'headline_optimizer':
             return (
                <>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Nome do Produto</label>
                        <input type="text" className="w-full bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none" placeholder="Ex: Tênis Ortopédico" value={dynamicFormData.productName || ''} onChange={e => handleInputChange('productName', e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Qual a promessa?</label>
                        <textarea className="w-full h-24 bg-[#0B0518] border-2 border-slate-700 rounded-xl p-4 text-sm font-medium text-white outline-none resize-none" placeholder="Ex: Acabar com dores nas costas em 3 dias" value={dynamicFormData.promise || ''} onChange={e => handleInputChange('promise', e.target.value)} />
                    </div>
                </>
             );
        default:
  ...
  return (
    <div className="flex h-screen bg-[#0B0518] text-white font-sans overflow-hidden relative selection:bg-purple-500/30">
      ...
      {/* MODAL DE CONFIRMAÇÃO DE DELETAR ITEM */}
      <ConfirmModal 
        isOpen={showDeleteModal}
        title="Excluir Item?"
        message="Tem certeza que deseja apagar este item da sua biblioteca? Esta ação não pode ser desfeita."
      ...
      <ConfirmModal 
        ...
        title="Limpar Campos?"
        message="Isso apagará tudo que você digitou no formulário atual para começar do zero."
        onConfirm={handleClearFormConfirm}
             ...
             <div key="history" className="flex-1 flex flex-col h-screen overflow-hidden bg-[#0B0518] animate-in fade-in slide-in-from-bottom-8 duration-500 ease-out">
              ...
              <div key={activeModule} className="flex flex-col lg:flex-row h-full overflow-hidden relative animate-in fade-in slide-in-from-bottom-8 duration-500 ease-out">
                  ...
                  <div className="w-full lg:w-[380px] bg-slate-950 border-r border-slate-800 flex flex-col h-auto lg:h-full z-20 shadow-2xl overflow-y-auto custom-scrollbar">
                        ...
                        <div className="p-6 bg-slate-900/30 border-t border-slate-800 mt-auto flex flex-col gap-3">
                            <button 
